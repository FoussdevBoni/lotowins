...
export default function TableDepots({user}) {
  ...
  React.useEffect(()=>{
    ...
    onValue(dataRef , (snapshot)=>{
      ...
      if (data) {
        ...
        const myDepots = usersData.filter(item=>item.deposant.id===user.id)
        
        setDepots(myDepots)

       const newDate = dateISO8601(new Date().toISOString());
  
  const newDepots = myDepots.filter((retrait) => {
